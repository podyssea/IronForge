...
export default function App() {
  ...
  </ScrollView><View style={{ position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "row", paddingTop: 15, paddingBottom: 30, backgroundColor: "#10151A" }}>{(["Today", "Plan", "Progress", "Profile"] as Tab[]).map(x => <Pressable key={x} style={{ flex: 1, alignItems: "center" }} onPress={() => { setTab(x); setPicker(false); }}><Text style={{ color: tab === x ? "#9DE5A8" : "#77818B", fontWeight: "900",fontSize:12 }}>{x}</Text></Pressable>)}</View></SafeAreaView>;
}
