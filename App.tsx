...
const STORAGE_KEY = "ironforge-workouts-v1";
const HISTORY_KEY = "ironforge-history-v1";
const PROGRAM_KEY = "ironforge-program-v1";
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([AsyncStorage.getItem(STORAGE_KEY), AsyncStorage.getItem(HISTORY_KEY), AsyncStorage.getItem(PROGRAM_KEY)]).then(([savedWorkouts, savedRecords, savedProgram]) => {
      if (savedWorkouts) setWorkouts(JSON.parse(savedWorkouts));
      if (savedRecords) setRecords(JSON.parse(savedRecords));
      if (savedProgram) { const program = JSON.parse(savedProgram); setTrainingDays(program.trainingDays); setPhase(program.phase); }
      setLoaded(true);
    }).catch(() => setLoaded(true));
  }, []);
  useEffect(() => { if (loaded) AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(workouts)); }, [workouts, loaded]);
  useEffect(() => { if (loaded) AsyncStorage.setItem(HISTORY_KEY, JSON.stringify(records)); }, [records, loaded]);
  useEffect(() => { if (loaded) AsyncStorage.setItem(PROGRAM_KEY, JSON.stringify({ trainingDays, phase })); }, [trainingDays, phase, loaded]);
