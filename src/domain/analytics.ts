import { SessionRecord, SetLog } from "./training";
...
export function calculateAnalytics(records: SessionRecord[], range: AnalyticsRange, plannedWeeklySessions: number, now = new Date()): TrainingAnalytics {
  ...
  const sets = selected.flatMap((record) => record.exercises).flatMap((exercise) => exercise.sets).filter((set) => set.completed).length;
  ...
  const volumeChangePercent = range === "all" ? undefined : comparePriorVolume(records, range, volume, now);
  return { sessions: selected.length, sets, volume, durationSeconds, consistencyPercent, volumeChangePercent, coachSummary: coachSummary(selected.length, consistencyPercent, volumeChangePercent) };
}
