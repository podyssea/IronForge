...
function SessionDetail({ record, records, onBack, onNotes, onDelete }: { record: SessionRecord; records: SessionRecord[]; onBack: () => void; onNotes: (notes: string) => void; onDelete: () => void }) {
  ...
  return <><Pressable onPress={onBack} style={styles.back}><Text style={styles.backText}>← BACK TO HISTORY</Text></Pressable><Text style={styles.kicker}>JOURNAL ENTRY</Text><Text style={styles.title}>{record.workoutTitle.split(" · ").pop()}</Text><Text style={styles.detailDate}>{formatDate(record.completedAt)} · {record.startedAt ? `${formatTime(record.startedAt)}–${formatTime(record.completedAt)}` : formatTime(record.completedAt)}</Text>
    ...
    {record.readiness && <View style={styles.readiness}><Text style={styles.readinessTitle}>READINESS · {record.readiness.score}% {record.readiness.level.toUpperCase()}</Text><Text style={styles.readinessText}>Energy {record.readiness.energy}/5 · Sleep {record.readiness.sleep}/5 · Soreness {record.readiness.soreness}/5</Text></View>}
...
const styles = StyleSheet.create({
  ...
  readiness: { backgroundColor: "#202a1d", borderLeftWidth: 3, borderLeftColor: "#d8ff38", borderRadius: 7, padding: 12, marginTop: 14 }, readinessTitle: { color: "#d8ff38", fontSize: 9, fontWeight: "900", letterSpacing: .7 }, readinessText: { color: "#aab3a7", fontSize: 10, marginTop: 5 },
  ...
  setRow: { flexDirection: "row", alignItems: "center", borderTopWidth: 1, borderTopColor: "#2b312b", minHeight: 39 }, skippedRow: { opacity: .48 }, setIndex: { color: "#8e978c", fontSize: 8, fontWeight: "900", width: "22%" }, setValue: { color: "#e5e9e2", fontSize: 12, fontWeight: "700", width: "45%" }, setStatus: { color: "#b9c99b", fontSize: 9, fontWeight: "800", flex: 1, textAlign: "right" }, skipped: { color: "#9b7770" }, comparison: { color: "#b9c99b", fontSize: 9, lineHeight: 14, marginTop: 10 },
